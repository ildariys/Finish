...
Процедура ВыполнитьБронирование(ДокументСсылка) Экспорт
	...
	Для каждого ТекСтрокаТовары Из ДокументСсылка.СписокПеремещаемыхКниг Цикл
		...
		Движение.Склад = ДокументСсылка.СкладОтправки;
...
Функция ДоступностьБрони(Ссылка)
	...
	// Получение остатков из регистра (на момент времени документа)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПеремещениеКнигСписокПеремещаемыхКниг.Книга КАК Книга,
		|	СУММА(ПеремещениеКнигСписокПеремещаемыхКниг.Количество) КАК Количество1
		|ПОМЕСТИТЬ ВТ_ТоварыДокумента
		|ИЗ
		|	Документ.ПеремещениеКниг.СписокПеремещаемыхКниг КАК ПеремещениеКнигСписокПеремещаемыхКниг
		|ГДЕ
		|	ПеремещениеКнигСписокПеремещаемыхКниг.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ПеремещениеКнигСписокПеремещаемыхКниг.Книга
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ПО ВТ_ТоварыДокумента.Книга = ВТ_Остатки.Книга
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Бронь КАК ВТ_Бронь
		|		ПО ВТ_ТоварыДокумента.Книга = ВТ_Бронь.Книга";
	
	Запрос.УстановитьПараметр("Период", Ссылка.Дата);
	Запрос.УстановитьПараметр("Склад", Ссылка.СкладОтправки);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.КоличествоДокумент > КоличествоОстатка Тогда
			...
			Сообщение.Текст = "Недостаточно товара " + Выборка.КнигаПредставление 
				+ " в количестве " + Дефицит + " шт";
			Сообщение.Сообщить();
		КонецЕсли;
