...
Процедура ПроведениеПоFIFOLIFO(УчетнаяПолитика)    
	...
	Запрос.Текст = 
		...
		|	ОстаткиНомеклатурыОстатки.Партия,
		|	ОстаткиНомеклатурыОстатки.Партия.Дата,
		|	ЕСТЬNULL(ОстаткиНомеклатурыОстатки.КоличествоОстаток, 0),
		|	ЕСТЬNULL(БронированиеТоваровОстатки.КоличествоЗабронированногоОстаток, 0)
		|ИЗ
		|	ТабДок КАК ТабДок
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.БронированиеТоваров.Остатки(&Момент, Книга В
		|			(ВЫБРАТЬ
		|				ТабДок.Книга
		|			ИЗ
		|				ТабДок)
		...
		|	РегистрНакопления.ОстаткиНомеклатуры.Остатки(&Момент, Книга В
		|		(ВЫБРАТЬ
		|			ТабДок.Книга
		|		ИЗ
		|			ТабДок)
	...
	СпособСписания = Константы.СпособСписания;
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл   
			...
			Если   ВыборкаНоменклатура.КолДок > (ВыборкаНоменклатура.КолОст - ВыборкаНоменклатура.БроньОст) Тогда 
				...
				Нехватка = ВыборкаНоменклатура.КолДок - ВыборкаНоменклатура.КолОст;
				
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = "Не хватает " +Нехватка+   " шт. товара "+ВыборкаНоменклатура.Номенклатура+
