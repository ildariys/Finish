...
Процедура ПрочитатьФайл_XLS_НаСервере(АдресДанных, Расширение)
	...
	Для НомерСтроки = 2 По КоличествоСтрок Цикл
		...
		НоваяСтрока.ГодВыпуска = ТабДок.Область(НомерСтроки, 8).Строка;
...
Процедура ЗаписатьДанныеНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		Массив = Новый Массив;
		Массив.Добавить(Выборка.Описание);
		ФорматированныйДокумент.Добавить(Массив[0]);
		ЗначениеОписание = Новый ХранилищеЗначения(ФорматированныйДокумент);
		СтруктураКниги.Вставить("Описание", ЗначениеОписание);
